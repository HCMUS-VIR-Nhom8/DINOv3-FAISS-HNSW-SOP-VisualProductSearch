"""
Re-ranking: tu danh sach ung vien (candidates) sau khi gop nhieu nguon + loc blacklist,
chon ra Top-K cuoi cung. Baseline dung lai diem cosine similarity da co; co san hook de
cong them diem tu "Text Re-ranking" (khop ten/mo ta san pham) dung theo Hinh 2.9.
"""
...
def merge_candidates(
    ...
    """Gop ung vien tu nhieu nguon (vd Static HNSW + Dynamic Buffer), giu diem cao nhat khi trung id."""
...
def text_rerank(
    ...
    """
    Ket hop diem hinh anh voi diem khop text (neu co query_text + metadata san pham).
        score_final = alpha * score_visual + (1 - alpha) * score_text
    Baseline dung khop tu khoa don gian (bag-of-words overlap); co the thay bang BM25
    (xem rank_bm25 trong requirements.txt) hoac cross-encoder khi can chinh xac hon.
    """
    ...
    final = alpha * scores + (1 - alpha) * text_scores_arr
